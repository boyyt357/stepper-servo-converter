...
// TMC UART Pins (Single Wire on GPIO 17 - Untouched per request)
const int RX_PIN = 17;
const int TX_PIN = 17;
...
// --- Core 1 RTOS Loop (Dedicated Dynamic Velocity Tracking) ---
void controlLoopTask(void * pvParameters) {
  ...
  for(;;) {
    ...
    // Safely copy shared parameters from Core 0
    portENTER_CRITICAL(&paramMux);
    float safeTarget = targetDeg;
    float safeKp = Kp, safeKi = Ki, safeKd = Kd;
    float safeTol = tolerance;
    float safeMaxSpeed = maxSpeedVal;
    int safeMode = controlMode;
    portEXIT_CRITICAL(&paramMux);

    // 1. Process Input Signals
    float rawInput = safeTarget; 
    if (safeMode == 1 && pulseWidth > 800 && pulseWidth < 2200) {
    ...
    if (safeMode != 0) {
      safeTarget = (filterAlpha * rawInput) + ((1.0 - filterAlpha) * safeTarget);
      portENTER_CRITICAL(&paramMux);
      targetDeg = safeTarget; // Update global state for UI tracking
      portEXIT_CRITICAL(&paramMux);
    }

    // 2. Closed-Loop PID (Velocity Mode)
    unsigned long now = micros();
    float dt = (now - lastPIDTime) / 1000000.0;
    if (dt <= 0.0) dt = 0.004; // Fallback to 4ms
    lastPIDTime = now;

    float error = safeTarget - currentDeg;

    // Check deadzone tolerance
    if (abs(error) < safeTol) {
      stepper->stopMove(); 
      integralError = 0.0; // Anti-windup
    } else {
      ...
      // Compute PID Output (Correction in Degrees Per Second -> Velocity)
      float pidOutputDegPerSec = (safeKp * error) + (safeKi * integralError) + (safeKd * derivative);

      // Convert Deg/Sec into Stepper Hz (Steps per Second)
      float speedCmdHz = (pidOutputDegPerSec * 200.0 * tmcMicrosteps * gearRatio) / 360.0;
      
      // Constrain speed to hardware limits
      if (speedCmdHz > safeMaxSpeed) speedCmdHz = safeMaxSpeed;
      if (speedCmdHz < -safeMaxSpeed) speedCmdHz = -safeMaxSpeed;

      // Apply Dynamic Velocity Target
      if (abs(speedCmdHz) > 0) {
        stepper->setSpeedInHz(abs(speedCmdHz));
        stepper->applySpeedAcceleration(); // Apply changes on the fly
        if (speedCmdHz > 0) {
          stepper->runForward();
        } else {
          stepper->runBackward();
...
void setup() {
  ...
  Wire.setTimeOut(20); // Fix: Prevent I2C RTOS bus lockup
  ...
  if (stepper) {
    ...
    stepper->setEnablePin(EN_PIN, false); // Fix: TMC2209 Active LOW
  ...
  server.on("/save", [](AsyncWebServerRequest *f){
    // Thread Safe and HTTP Parameter-Safe Saving
    portENTER_CRITICAL(&paramMux);
    
    if (f->hasParam("sp")) maxSpeedVal = f->getParam("sp")->value().toFloat();
    if (f->hasParam("ac")) accelVal = f->getParam("ac")->value().toFloat();
    if (f->hasParam("t")) tolerance = f->getParam("t")->value().toFloat();
    if (f->hasParam("max")) maxDeg = f->getParam("max")->value().toFloat();
    if (f->hasParam("r")) gearRatio = f->getParam("r")->value().toFloat();
    
    if (f->hasParam("tmcC")) tmcCurrent = f->getParam("tmcC")->value().toInt();
    if (f->hasParam("tmcM")) tmcMicrosteps = f->getParam("tmcM")->value().toInt();
    if (f->hasParam("tmcS")) tmcStealth = f->getParam("tmcS")->value().toInt() == 1;

    if (f->hasParam("kp")) Kp = f->getParam("kp")->value().toFloat();
    if (f->hasParam("ki")) Ki = f->getParam("ki")->value().toFloat();
    if (f->hasParam("kd")) Kd = f->getParam("kd")->value().toFloat();
    
    integralError = 0.0; // Reset anti-windup on save
    portEXIT_CRITICAL(&paramMux);

    // Mode handling (Outside critical block)
    if (f->hasParam("m")) {
